...
- Packed doc-block batches are now supported for `hf_deberta_v2` through a segment-aware flash route. The collator keeps compact `doc_ids`; compile metadata expands those ids into fixed-shape segment descriptors outside the graph; and an opaque doc-block custom op repacks contiguous document spans into a ragged batch for the tuned varlen kernels instead of materializing a dense pairwise mask.
