...
- Packed doc-block batches are now supported for `hf_deberta_v2` with a JSON-table regime split. Default packed doc-block training uses the segment-aware flash route at `1024` and longer contexts: the collator stores compact `doc_ids`, compile metadata expands them into fixed-shape segment descriptors outside the graph, and an opaque doc-block custom op repacks contiguous document spans into a ragged batch for the tuned varlen kernels instead of materializing a dense pairwise mask. The dense flash-with-bias doc-block route remains available only as an explicit kernel experiment because local RTD validation showed the dense route can collapse discriminator learning even when short synthetic parity checks pass.
...
- The opt-in packed-docblock `1024` dense-bias path has an exact-match repo-local backward specialization on top of that tuning seam. For non-causal bf16/fp16 `D=64` runs with a full dense `(B,H,1024,1024)` additive bias tensor, a matching `bias_docblock_specialized` entry in `src/deberta/modeling/flashdeberta_kernel_tuning.json` lets `src/deberta/modeling/flashdeberta_bias_op.py` dispatch dedicated `_bwd_kv_kernel_docblock1024` and `_bwd_q_kernel_docblock1024` Triton kernels instead of the more generic FlashDeBERTa local-bias backward launcher. Outside that narrow table policy, the wrapper still falls back to the generic raw bias backward kernels and their normal tuning path.
...
- TODO (docblock compile ownership): move doc-block segment host stats into tensor/custom-op-owned metadata. Current Python scalar stats can force Dynamo recompiles on ragged packed batches, and local `1024` packed-docblock validation shows the safe segment route is correctness-preserving but slower than eager.
