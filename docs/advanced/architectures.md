...
- Packed doc-block batches are now supported for `hf_deberta_v2` with a JSON-table route policy. The safe default keeps packed doc-block training on the segment-aware `docblock` route at all shipped sequence lengths. The dense flash-with-bias `docblock_bias` route remains available only as an explicit exact-length validation override through `model.hf.flash.docblock_bias_seq_len`. The collator stores compact `doc_ids`; compile metadata expands them either into fixed-shape segment descriptors for the ragged route or into a dense pairwise keep mask when the experimental bias route is explicitly selected.
...
- The packed-docblock `1024` dense-bias path has an exact-match repo-local backward specialization on top of that tuning seam. For non-causal bf16/fp16 `D=64` runs with a full dense `(B,H,1024,1024)` additive bias tensor, matching `bias_docblock_specialized` entries in `src/deberta/modeling/flashdeberta_kernel_tuning.json` let `src/deberta/modeling/flashdeberta_bias_op.py` dispatch dedicated `_bwd_kv_kernel_docblock1024` and `_bwd_q_kernel_docblock1024` Triton kernels instead of the more generic FlashDeBERTa local-bias backward launcher. This route is experimental and opt-in until real-batch RTD parity and convergence are proven. Outside that narrow table policy, the wrapper still falls back to the generic raw bias backward kernels and their normal tuning path.
- The position-bias attention backward reduces dense-bias gradients in fp32 before returning positional-score gradients. This keeps the experimental dense route conservative while its real-batch RTD behavior is being isolated.
...
- TODO (docblock segment-route performance): continue reducing the segment-aware `docblock` route's varlen backward overhead. It is the correctness-preserving default for packed doc-block training, but current local evidence shows it still trails eager throughput at `1024`; do not promote dense `docblock_bias` as a default until RTD convergence and parity are proven on real packed batches.
