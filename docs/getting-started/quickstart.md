...
Packed doc-block batches now use the segment-aware flash custom op by default, including exact packed `1024` batches. The dense flash-with-bias doc-block route remains available for explicit kernel experiments, but local RTD validation showed it can collapse discriminator learning, so do not use it as a training default. The collator keeps compact `doc_ids`; compile metadata expands them into fixed-shape segment descriptors for the ragged route, or into a dense pairwise keep mask only when the experimental bias route is explicitly selected. Leave the flash route fields unset to use the JSON table. Opt into the short-sequence bias route with `--model.hf.flash.docblock_bias_seq_len <len>` only after parity and training validation; set `--model.hf.flash.local_bias_max_batch_size 0` to disable the small-batch dense local-bias route.
