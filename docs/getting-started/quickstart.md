...
For exact same-directory continuation, keep `train.max_steps` unchanged, set `train.checkpoint.resume_from_checkpoint` to `auto`, set `train.checkpoint.resume_data_strategy` to `replay`, and leave both output directories unchanged. The scheduler is defined by the original step horizon, so changing `train.max_steps` is rejected rather than silently changing the resumed optimization schedule. Replay reconstructs loader/worker data position without consuming the checkpoint-restored process RNG. Then run:
