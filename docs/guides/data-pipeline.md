...
Whole-word n-gram selection walks context windows of `n * int(1 / mlm_probability)` word groups. Special tokens split those windows and marked spans, so an n-gram never crosses a packed-document boundary. Every partial context window is taken with probability proportional to its width; if all are rejected, one word group is selected uniformly across the row. Marked spans are consumed in random order when stopping at the budget. These rules keep the mask rate positionally uniform without privileging the first document in a packed row.
