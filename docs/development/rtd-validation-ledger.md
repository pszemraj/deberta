...
It completed 1.638B input tokens in 12h39m18s at approximately 36.0k tok/s. The throughput drop relative to the run above is not attributable to the collator change: the fix leaves the per-row masked-token count identical, since the old path also trimmed its selection back to the same budget, and the GPU stayed compute-bound at 97-98% utilization throughout, which rules out added host-side collator cost. The host moved from driver `575.64.03` and kernel 6.14 to `595.71.05` / CUDA 13.2 and kernel 6.17 between the two runs; that is the likely cause but was not isolated. All ten checkpoints and the `final_hf` export committed successfully, with zero non-finite elements. The final training window reported generator CE `1.791`, discriminator BCE `0.1945`, prior gain `0.0609`, and replacement rate `0.0707`.
