...
FIELD_DOCS = {
    ...
    "model.hf.flash.docblock_bias_seq_len": "Only used when `model.hf.attention_impl=flash`. Exact-length dense doc-block route override. `null` uses the JSON table, whose safe default keeps packed doc-block batches on segment-aware `docblock`. Set a positive exact sequence length only for isolated dense `docblock_bias` validation; set `0` to force-disable dense doc-block routing.",
