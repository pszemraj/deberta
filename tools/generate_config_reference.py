...
FIELD_DOCS = {
    "model.backbone_type": "`hf_deberta_v2` is the native DeBERTa-v2/v3 parity backbone. `rope` is the experimental RoPE/RMSNorm/SwiGLU-capable path.",
    ...
    "model.hf.model_size": "Native HF DeBERTa-v2/v3 model size preset. Has no effect for `rope`; changing it from the default in that mode emits a warning.",
    "model.hf.attention_kernel": "Native eager DeBERTa attention implementation choice. Has no effect for `rope`; changing it from the default in that mode emits a warning.",
    "model.hf.attention_impl": "`eager` is the portability default. `flash` is valid only with `model.backbone_type=hf_deberta_v2`, requires dropout disabled, and uses the JSON policy under `model.hf.flash.*`.",
    ...
    "model.rope.norm_arch": "`post` applies RMSNorm after each residual addition. `keel` enables KEEL scaling and activates `keel_alpha_*` controls.",
    "model.rope.norm_eps": "RoPE norm epsilon. Must be positive in practice; default 1e-6 is recommended for bf16 stability.",
    "model.rope.keel_alpha_init": "Only used when `model.rope.norm_arch=keel`; a non-default value with `post` has no effect and emits a warning.",
    "model.rope.keel_alpha_learnable": "Only used when `model.rope.norm_arch=keel`; enabling it with `post` has no effect and emits a warning.",
    ...
    "model.rope.swiglu_adjust_intermediate": "Only used when `model.rope.ffn_type=swiglu`; changing it from the default with `mlp` has no effect and emits a warning.",
    ...
    "train.sdpa_kernel": "PyTorch SDPA kernel policy. Only affects `rope` with `model.rope.attention_implementation=sdpa`. A non-default value for `hf_deberta_v2` has no effect and emits a warning; a non-default value with RoPE eager attention is invalid. `flash` is invalid with packed cross-document blocking.",
    ...
    "train.compile.mode": "Torch compile mode. Only used when `train.compile.enabled=true`; a non-default value while compile is disabled has no effect and emits a warning.",
    ...
    "train.checkpoint.export_hf_final": "Attempts to export final discriminator artifacts after successful training. Export failures are logged without changing the training exit status. Disable for benchmark/probe runs.",
