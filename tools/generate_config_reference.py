#!/usr/bin/env python3
"""Generate the single-page training config reference from dataclass schema."""
...
FIELD_DOCS = {
    ...
    "model.hf.attention_impl": "`eager` is the portability default. `flash` is valid only with `model.backbone_type=hf_deberta_v2`, requires dropout disabled, and uses the JSON route/kernel policy under `model.hf.flash.*`; validate throughput on new GPU types before treating measured table defaults as portable.",
    ...
    "model.hf.flash.docblock_bias_seq_len": "Only used when `model.hf.attention_impl=flash`. Exact-length dense doc-block route override. `null` uses the JSON table, which selects dense `docblock_bias` for measured packed `1024`/`2048`/`4096` buckets and uses ragged `docblock` elsewhere. Set a positive exact sequence length to force dense only at that length; set `0` to force-disable dense doc-block routing and use ragged segment metadata.",
    "model.hf.flash.local_bias_seq_len": "Only used when `model.hf.attention_impl=flash`. Dense local-bias route seq-len override for plain (non-packed) batches. `null` uses table policy; `0` disables the local-bias route; a positive value allows it only at that exact sequence length. Independent of `model.hf.flash.docblock_bias_seq_len`.",
    "model.hf.flash.local_bias_max_batch_size": "Only used when `model.hf.attention_impl=flash`. Dense local-bias route cap. `null` uses table policy; `0` disables. Dense routes can be memory-heavy and hardware-sensitive.",
