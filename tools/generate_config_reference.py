...
FIELD_DOCS = {
    ...
    "model.hf.attention_impl": "`eager` is stable. `flash` is experimental, only valid with `model.backbone_type=hf_deberta_v2`, and requires dropout disabled.",
    ...
    "model.hf.flash.docblock_bias_seq_len": "Only used when `model.hf.attention_impl=flash`. Experimental dense doc-block route opt-in. `null` uses the table, whose safe default is segment-aware docblock. Set a positive exact length only after local parity and RTD validation; `0` disables.",
